-- 04_build_gold_bi_extensions_v13.sql
-- Build the lean Gold financial layer, investor return layer, and BI marts.
-- v13 adds a host_since-based financing vintage proxy so older operators can carry
-- cheaper acquisition assumptions and a year-appropriate mortgage rate.
...
INSERT INTO gold_investor_assumptions (
    ...
    (1, 'optimized_operator_2026', 'urban', 'company_optimized', 90000, 18000, 3500, 10000, 0.05, 0.0035, 1020, 300, 0.17, 0.90, 0.0375, 30, 1, 'Optimized Portuguese investor setup for urban markets with deductible business expenses and 90% bank financing.'),
    (2, 'optimized_operator_2026', 'city',  'company_optimized', 105000, 22000, 4500, 12000, 0.05, 0.0038, 1020, 300, 0.17, 0.90, 0.0375, 30, 1, 'Optimized Portuguese investor setup for city markets with deductible business expenses and 90% bank financing.'),
    (3, 'optimized_operator_2026', 'beach', 'company_optimized', 115000, 25000, 5000, 13000, 0.05, 0.0038, 1020, 300, 0.17, 0.90, 0.0390, 30, 1, 'Optimized Portuguese investor setup for beach markets with deductible business expenses and 90% bank financing.');
...
SELECT
    ...
    ROUND(rf.estimated_loan_amount * rf.applied_annual_interest_rate, 4) AS annual_interest_cost,
    ...
    ROUND(
        rf.estimated_total_project_capex / NULLIF(
            rf.projected_noi - (rf.imi_cost + rf.accounting_cost + rf.licensing_cost) - rf.annual_debt_service
            - (
                GREATEST(
                    rf.projected_noi - (rf.imi_cost + rf.accounting_cost + rf.licensing_cost) - rf.annual_debt_service,
...
INSERT INTO gold_mart_property_bi (
    ...
    review_scores_cleanliness, review_scores_location, review_scores_value, scenario_name,
    ...
    annual_debt_service, monthly_debt_service, tax_due_estimate, monthly_tax_due_estimate,
    net_income_after_tax, monthly_net_income_after_tax, net_cash_flow_after_debt, monthly_net_cash_flow_after_debt,
...
SELECT
    ROW_NUMBER() OVER (ORDER BY fls.property_key) AS property_bi_key,
    fls.global_property_id,
    fls.property_id,
    fls.property_key,
    fls.host_id,
...
INSERT INTO gold_mart_neighbourhood_bi (
    ...
    region_group, market_segment, market_type, coastal_flag, urban_flag, listing_count, avg_price,
    ...
    avg_estimated_total_project_capex, avg_annual_debt_service, avg_monthly_debt_service, avg_tax_due_estimate, avg_monthly_tax_due_estimate,
...
SELECT
    ROW_NUMBER() OVER (ORDER BY location_key) AS neighbourhood_bi_key,
    location_key,
    city,
...
GROUP BY
    ...
    market_segment, market_type, coastal_flag, urban_flag, snapshot_date_key, snapshot_date;

INSERT INTO gold_mart_host_bi (
    host_bi_key, host_key, host_id, property_count, city_count, avg_price, avg_occupancy_rate,
    ...
    avg_annual_debt_service, avg_monthly_debt_service, avg_tax_due_estimate, avg_monthly_tax_due_estimate, avg_net_income_after_tax, avg_monthly_net_income_after_tax,
    avg_net_cash_flow_after_debt, avg_monthly_net_cash_flow_after_debt, avg_cash_on_cash_return, avg_review_score_rating,
    avg_platform_count, superhost_flag, snapshot_date_key, snapshot_date
)
SELECT
    ROW_NUMBER() OVER (ORDER BY host_key) AS host_bi_key,
    host_key,
    host_id,
    ...
    snapshot_date
FROM gold_mart_property_bi
GROUP BY host_key, host_id, snapshot_date_key, snapshot_date;

INSERT INTO gold_mart_market_bi (
    market_bi_key, city, region_group, market_segment, market_type, room_type, accommodates_bucket,
...
SELECT
    ROW_NUMBER() OVER (ORDER BY snapshot_date_key, city, region_group, market_type, room_type, accommodates_bucket) AS market_bi_key,
    city,
    region_group,
    market_segment,
    market_type,
...
GROUP BY snapshot_date_key, snapshot_date, city, region_group, market_segment, market_type, room_type, accommodates_bucket;
