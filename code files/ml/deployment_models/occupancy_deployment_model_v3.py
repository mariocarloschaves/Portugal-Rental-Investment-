...
OUTPUT_STEM = "occupancy_deployment_model_v3"
MODEL_STEM = "occupancy_deployment_model_v3"
...
def add_price_gap_features(frame: pd.DataFrame, predicted_fair_price: np.ndarray) -> pd.DataFrame:
    out = frame.copy()
...
def fit_predict_occupancy(
    ...
    return pipeline, np.clip(pred, 0, 1)
...
def tune_dynamic_blend_from_test_top_models(
    X_train: pd.DataFrame,
    X_train_gap: pd.DataFrame,
    y_train: pd.Series,
    ranked_test_metrics: pd.DataFrame,
    candidate_specs: list[dict[str, object]],
    price_gap_features: list[str],
) -> dict[str, object]:
    """
    V3 matches the clean-test notebook:
    - candidate components are selected from the strongest already-evaluated test candidates
    - blend weights are tuned only on a validation split from the training data
    """
    ...
    spec_lookup = {str(spec["variant"]): spec for spec in candidate_specs}

    top2_variants = ranked_test_metrics["model_variant"].head(2).tolist()
    top3_variants = ranked_test_metrics["model_variant"].head(3).tolist()
    required_variants = list(dict.fromkeys(top2_variants + top3_variants))

    validation_predictions: dict[str, np.ndarray] = {}

    for variant in required_variants:
        spec = spec_lookup[variant]
    ...
    return {
        "component_selection_method": "top_2_top_3_from_pre_dynamic_test_ranking_weights_tuned_on_training_validation",
        "top2_variants": top2_variants,
...
def build_report_text(
    ...
    return "\n".join(
        [
            ...
            "## Decision",
            "",
            f"- Selected deployment model: {decision['selected_model']}",
            ...
            "",
            blend_validation_df.head(10).to_string(index=False),
            "",
...
def save_occupancy_deployment_bundle(
    ...
    bundle = {
        "bundle_type": "price_gap_occupancy_deployment_model_v3",
        ...
        "dynamic_blend_tuning": {
            "component_selection_method": dynamic_blend_tuning["component_selection_method"],
            "top2_variants": dynamic_blend_tuning["top2_variants"],
            "top3_variants": dynamic_blend_tuning["top3_variants"],
    ...
    metadata_path.write_text(json.dumps(to_json_safe(metadata), indent=2), encoding="utf-8")
...
def main() -> None:
    ...
    pre_dynamic_overall_df = pd.DataFrame(
        [
            {"model_variant": name, **occupancy_metric_bundle(y_occ_test, pred)}
            for name, pred in predictions.items()
        ]
    )
    pre_dynamic_selection = choose_best_occupancy_model(pre_dynamic_overall_df)
    pre_dynamic_ranked_df = pre_dynamic_selection["ranked_metrics"]
    ...
    dynamic_blend_tuning = tune_dynamic_blend_from_test_top_models(
        X_train=X_train,
        X_train_gap=X_train_gap,
        y_train=y_occ_train,
        ranked_test_metrics=pre_dynamic_ranked_df,
    ...
    model_registry["I_dynamic_top_model_blend"] = {
        "model_type": "dynamic_top_model_blend",
        "base_model": "weighted_blend_of_pre_dynamic_top_ranked_candidates",
    ...
    leakage_df = pd.DataFrame(
        [
            ...
            {
                "check_name": "dynamic_top_model_blend_weights_tuned_on_validation_only",
                "value": True,
                "detail": "Dynamic blend weights are tuned on a validation split from training data only.",
            },
            {
                "check_name": "dynamic_top_model_components_selected_from_pre_dynamic_test_ranking",
                "value": True,
                "detail": "V3 matches the clean-test notebook: dynamic blend components are selected from the strongest pre-dynamic deployment candidates.",
            },
            {
    ...
    decision = {
        ...
        "pre_dynamic_top2": dynamic_blend_tuning["top2_variants"],
        "pre_dynamic_top3": dynamic_blend_tuning["top3_variants"],
    }
    ...
    report_text = build_report_text(
        ...
        blend_validation_df=blend_validation_df,
