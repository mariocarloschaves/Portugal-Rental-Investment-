...
OUTPUT_STEM = "price_deployment_model_v1"
MODEL_STEM = "nightly_price_deployment_model_v1"
...
def build_validation_predictions(frame: pd.DataFrame) -> tuple[SplitBundle, dict[str, object], dict[str, object], dict[str, object]]:
    ...
    segmented_config = select_best_segmented_config(split)["best_config"]
...
def tune_segment_specific_weights(
    ...
    for normal_weight in WEIGHT_GRID_SEGMENT:
        for luxury_weight in WEIGHT_GRID_SEGMENT:
            weight_vector = np.where(predicted_segments == "luxury", luxury_weight, normal_weight)
            pred, metrics = evaluate_weighted_prediction(y_valid, baseline_pred, segmented_pred, weight_vector)
            row = {
...
def tune_bucket_specific_weights(
    ...
    predicted_bucket_valid = assign_price_bucket(pd.Series(baseline_pred), bucket_edges).astype(str).to_numpy()
    ...
    for bucket_name in ["low_price", "medium_price", "high_price", "luxury_extreme_price"]:
        ...
        for weight in WEIGHT_GRID_GLOBAL:
            ...
            bucket_rmse = metrics["rmse"]
            if bucket_rmse < best_bucket_rmse:
                best_bucket_rmse = bucket_rmse
    ...
    pred, metrics = evaluate_weighted_prediction(y_valid, baseline_pred, segmented_pred, final_weight_vector)
    result_row = {
...
def tune_blend_strategies(
    ...
    predicted_segments = np.where(segmented_validation["luxury_test_proba"] >= 0.5, "luxury", "normal")

    global_result = tune_global_weight(split.y_valid, baseline_pred, segmented_pred)
    segment_result = tune_segment_specific_weights(split.y_valid, baseline_pred, segmented_pred, predicted_segments)
    ...
    strategy_df = pd.DataFrame(strategy_rows).sort_values(["rmse", "mae", "r2"], ascending=[True, True, False]).reset_index(drop=True)
    blend_only_df = strategy_df[strategy_df["strategy"].isin(["global_weight", "segment_specific_weight", "bucket_specific_weight"])].reset_index(drop=True)
    best_strategy_name = blend_only_df.iloc[0]["strategy"]
    ...
    return {
        ...
        "chosen_blend": lookup[str(best_strategy_name)],
    }
...
def build_weight_vector_for_test(
    ...
    if strategy_name == "segment_specific_weight":
        ...
        weight_vector = np.where(predicted_segments == "luxury", luxury_weight, normal_weight).astype(float)
        detail_rows.extend(
            [
    ...
    if strategy_name == "bucket_specific_weight":
        ...
        predicted_buckets = assign_price_bucket(pd.Series(baseline_test_pred), bucket_edges).astype(str).to_numpy()
        ...
        for bucket_name, bucket_weight in bucket_weights.items():
            ...
            detail_rows.append({"scope": "predicted_price_bucket_from_baseline", "value": bucket_name, "weight": bucket_weight})
        return weight_vector, pd.DataFrame(detail_rows)
...
def build_test_prediction_frame(
    ...
    predicted_bucket_by_baseline = assign_price_bucket(pd.Series(baseline_test_pred), bucket_edges).astype(str).to_numpy()
...
def build_metric_tables(
    ...
    overall_rows = [
        {"model_variant": "baseline_single_hgb_log_observed_full", **metric_bundle(split.y_test, baseline_test_pred)},
        {"model_variant": "segmented_best_validation", **metric_bundle(split.y_test, segmented_test_pred)},
        {"model_variant": "blended_candidate", **metric_bundle(split.y_test, blended_test_pred)},
    ]
    overall_df = pd.DataFrame(overall_rows)

    actual_segment = pd.Series(np.where(split.y_test >= threshold_value, "luxury", "normal"), index=split.y_test.index)
    train_segment = pd.Series(np.where(split.y_train_all >= threshold_value, "luxury", "normal"), index=split.y_train_all.index)
...
def save_candidate_bundle(
    ...
    bundle = {
        "bundle_type": "leakage_free_blended_price_model",
        "selected_model": "blended_candidate",
...
def build_report_text(
    ...
    blended_row = overall_df.loc[overall_df["model_variant"] == "blended_candidate"].iloc[0]
    promotion_text = "YES" if promoted else "NO"
    lines = [
        "# Price Deployment Model V1",
        "",
        "## Decision",
        "",
        f"- Use blended price deployment model over current baseline: {promotion_text}",
        ...
        "",
        f"- Baseline R2={baseline_row['r2']:.6f}, blended R2={blended_row['r2']:.6f}",
        f"- Baseline RMSE={baseline_row['rmse']:.6f}, blended RMSE={blended_row['rmse']:.6f}",
        f"- Baseline MAE={baseline_row['mae']:.6f}, blended MAE={blended_row['mae']:.6f}",
        "",
        "## Segment metrics",
        "",
...
def main() -> None:
    ...
    strategy_tuning = tune_blend_strategies(split, baseline_validation, segmented_validation)
    ...
    blended_test_pred, blended_metrics = evaluate_weighted_prediction(
        split.y_test,
        baseline_test_model["pred"],
        segmented_test_model["pred"],
        weight_vector_test,
        ...
        [
            {
                ...
                "value": True,
                "detail": f"Threshold quantile {segmented_config['threshold_q']} was selected on validation and the final threshold value {segmented_test_model['threshold_value']:.4f} was computed from y_train_all only.",
            },
            {
            ...
            },
            {
                ...
                "value": True,
                "detail": f"The chosen blend strategy {chosen_strategy['config']['strategy']} and its weights were tuned using validation predictions only.",
            },
            {
            ...
            {
                "check_name": "baseline_model_preserved_as_fallback",
                "value": True,
                "detail": "The current HGB log-price baseline is stored inside the blended bundle and remains the fallback model.",
            },
        ]
    )
    ...
    promoted = bool(
        blended_metrics["r2"] > overall_df.loc[overall_df["model_variant"] == "baseline_single_hgb_log_observed_full", "r2"].iloc[0]
        and blended_metrics["rmse"] < overall_df.loc[overall_df["model_variant"] == "baseline_single_hgb_log_observed_full", "rmse"].iloc[0]
        and blended_metrics["mae"] < overall_df.loc[overall_df["model_variant"] == "baseline_single_hgb_log_observed_full", "mae"].iloc[0]
    )
    ...
    for filename, frame in output_files.items():
        frame.to_csv(Path(__file__).resolve().parent / filename, index=False)

    (Path(__file__).resolve().parent / f"{OUTPUT_STEM}_report.md").write_text(report_text, encoding="utf-8")
    (Path(__file__).resolve().parent / f"{OUTPUT_STEM}_summary.json").write_text(json.dumps(summary_payload, indent=2), encoding="utf-8")
